...
extern int nes_menu_show(void);
extern void nes_save_state(const char *path);
extern void nes_load_state(const char *path);
extern void save_sram(void);
...
void nes_emulate(void)
{
    ...
    {
        if (showOverlay) {
            int ret = nes_menu_show();
            showOverlay = 0;
            if (ret == 3) {           /* NES_MENU_RESET */
                nes_reset(SOFT_RESET);
            }
            if (ret == 4) {           /* NES_MENU_QUIT */
                save_sram();
                main_quit();
            }
            if (ret == 1 || ret == 2) { /* SAVE or LOAD */
                extern const char *nes_platform_rom_path(void);
                extern void nes_platform_save_path(const char *rom_path,
                                                    char *out, size_t out_len);
                extern void nes_save_state(const char *path);
                extern void nes_load_state(const char *path);
                char sp[512];
                nes_platform_save_path(nes_platform_rom_path(), sp, sizeof(sp));
                if (ret == 1) nes_save_state(sp);
                if (ret == 2) nes_load_state(sp);
            }
            extern void nes_display_write(const uint8_t *data, int scale);
        ...
        if (frame == 60)
        {
            ...
            printf("HEAP:0x%lx, FPS:%f\n", (unsigned long)esp_get_free_heap_size(), fps);
