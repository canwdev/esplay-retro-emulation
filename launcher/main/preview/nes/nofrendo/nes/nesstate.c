...
static const char *s_save_name = NULL;

void nes_set_save_path(const char *path) {
  strlcpy(g_save_path, path, sizeof(g_save_path));
}

void nes_save_state(const char *path) {
  if (!path || !path[0]) return;
  state_save((char *)path);
}

void nes_load_state(const char *path) {
  if (!path || !path[0]) return;
  FILE *f = fopen(path, "rb");
  if (!f) return;
  fclose(f);
  state_load((char *)path);
}

void save_sram(void) {
  if (g_save_path[0])
    state_save(g_save_path);
}

void load_sram(void) {
  if (!g_save_path[0]) return;
  printf("load_sram: trying %s\n", g_save_path);
  FILE *f = fopen(g_save_path, "rb");
  if (!f) {
    printf("load_sram: no save file\n");
    return;
  }
  fclose(f);
  state_load(g_save_path);
}
