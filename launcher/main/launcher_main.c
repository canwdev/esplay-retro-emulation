...
static void lv_show_battery(void) {
  ...
  const char *charge_status = (bat.state == CHARGING)       ? "Charging"
...
static void list_items_event_handler(lv_event_t *e) {
  ...
  if (code == LV_EVENT_KEY) {
    ...
    // Find the label child of the button (List buttons have an icon and a
    // label)
    lv_obj_t *label = NULL;
    for (uint32_t i = 0; i < lv_obj_get_child_cnt(obj); i++) {
      lv_obj_t *child = lv_obj_get_child(obj, i);
...
int filechooser_filter_glob(const char *name, void *filterarg) {
  ...
  for (const char *p = glob; p != NULL; p = strchr(p, ',')) {
    ...
    // Zero-terminate at ','. Make sure that worst-case it's terminated at the
...
static int nextFdFileForFilter(int fd, fc_filtercb_t filter, void *filterarg,
                               const char **name) {
  while (1) {
...
static void lv_add_file_list(lv_obj_t *list, void *extension) {
  ...
  while (1) {
    ...
    if (fd == APPFS_INVALID_FD || !name)
      break;
    // appfsEntryInfo is called inside nextFdFileForFilter, so name should be
    // valid
...
static lv_obj_t *create_home_button(lv_obj_t *parent,
  ...
  if (!style_inited) {
    ...
    lv_style_set_bg_grad_color(&style_def,
...
static void lv_create_homescreen() {
  ...
  lv_obj_set_style_bg_color(ui_state.screen,
                            lv_palette_darken(LV_PALETTE_BLUE, 5), 0);
  lv_obj_set_style_bg_opa(ui_state.screen, LV_OPA_COVER, 0);
  lv_obj_set_style_bg_grad_color(ui_state.screen,
                                 lv_palette_lighten(LV_PALETTE_BLUE, 3), 0);
  lv_obj_set_style_bg_grad_dir(ui_state.screen, LV_GRAD_DIR_VER, 0);

  ui_state.time_label = lv_label_create(ui_state.screen);
  lv_label_set_text(ui_state.time_label, buffer);
  lv_obj_set_style_text_color(ui_state.time_label,
                              lv_palette_lighten(LV_PALETTE_GREY, 5), 0);
  ...
  lv_label_set_text(ui_state.battery_label, LV_SYMBOL_BATTERY_FULL);
  lv_obj_set_style_text_color(ui_state.battery_label,
                              lv_palette_lighten(LV_PALETTE_GREY, 5), 0);
  lv_obj_align(ui_state.battery_label, LV_ALIGN_TOP_RIGHT, 0, 0);

  ui_state.menu_selected_label = lv_label_create(ui_state.screen);
  lv_label_set_text(ui_state.menu_selected_label, "APPLICATION");
  lv_obj_set_style_text_color(ui_state.menu_selected_label,
                              lv_palette_lighten(LV_PALETTE_YELLOW, 4), 0);
